...
# 	sed -i 's/^OpenGLIsUnsafe=true/OpenGLIsUnsafe=false/g' $XDG_CONFIG_HOME/kwinrc
	...
	then
		convert $rc/Pictures/Arch/Arch.png -resize $screen RGB:- | i3lock --raw $screen:rgb -efi /dev/stdin -c 000000FF --indicator --ringver-color=ffae42ff --insidever-color=ffae42ff --inside-color=00000000 --line-uses-inside --verif-text="Verifying..." --wrong-text="Access Denied" --ring-width 20 --radius 120 #--no-verify
	elif [ "$1" == "t" ]
	then
